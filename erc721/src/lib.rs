...
const MIN_LEDGER: u32 = 1000;
const HIGH_EXPIRATION_WATERMARK: u32 = 10000;
...
impl ERC721 for ERC721Contract {
    fn balance_of(env: Env, owner: Address) -> u32 {
        DataKey::Balance(owner)
            .bump(&env, MIN_LEDGER, HIGH_EXPIRATION_WATERMARK)
            .get(&env)
            .unwrap_or(0)
    }
    ...
    fn approve(
        ...
        if let Some(to_approve) = operator {
            ...
            DataKey::Approved(token_id).bump_until(&env, expiration_ledger, HIGH_EXPIRATION_WATERMARK);
        } else {
            DataKey::Approved(token_id).remove(&env);
        }
    }
    fn set_approval_for_all(
        ...
        if approved {
            key.set(&env, &true);
            key.bump_until(&env, expiration_ledger, HIGH_EXPIRATION_WATERMARK);
        } else {
            key.remove(&env);
        }
    }
...
impl ERC721Contract {
    pub fn initialize(
        ...
        env.storage().instance().bump(10000, 15000);
